                ...
                <div class="message-payload">
                  ...
                  <div v-if="message.type === 'text'" class="content-text" style="flex-direction: column;">
                    ...
                    <div class="fz-box">
                      ...
                      <i class="el-icon-warning-outline" title="举报" style="cursor: pointer;" @click="clickReport(item)"></i>
                      <i class="el-icon-connection" title="引用" style="cursor: pointer;" @click="clickReply(item)"></i>
                    </div>
                  </div>
                  <!-- 内容 结束 -->
                  <!-- 引用内容 开始 -->
                  <div v-if="message.type === 'texts'" style="position: relative;">
                    ...
                    <div style="font-size: 12px;color: #555;margin: 4px 0;">
                      <div style="message-quote" v-html="emoji.decoder.decode(message.payload.quote)"></div>
                    </div>
                  </div>
                  <!-- 引用内容 结束 -->
                  <!-- 音视频 开始 -->
    ...
    <div class="chat-footer" :style="`height:${is_pop == 'pop'?'120':'140'}px;`">
      ...
      <div class="action-box" v-else>
        ...
        <div class="input-box" @click.stop="clickInput">
          ...
          <!-- 引用的消息 -->
          <div class="reply-message" v-if="quoteMessageShow">{{quoteMessage}}</div>
        </div>
        <div class="send-box">
    ...
    methods: {
      ...
      clickReply(item){
        if(item.senderId == ('u_'+localStorage.getItem('realUid'))){
          this.quoteMessage = localStorage.getItem('name') + item.payload.text;
        }else{
